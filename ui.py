...
class CreateClientWindow(Toplevel, CenterWidgetMixin):
    ...
    def build(self):
        ...
        Label(frame, text="DNI (2 ints y 1 upper chars)").grid(row=0, column=0)
    ...
    def validate(self, event, index):
        ...
        event.widget.configure({"highlightbackground": "Green", "bg": "PaleGreen1"} if valido else {"highlightbackground": "Red", "bg": "LightSalmon1"})
        #Carbiar el estado del bboton en base a las validaciones
        self.validaciones[index]=valido
        self.crear.config(state=NORMAL if self.validaciones==[1,1,1] else DISABLED)

class EditCreateClientWindow(Toplevel, CenterWidgetMixin):
    def __init__(self, parent):
        super().__init__(parent)
